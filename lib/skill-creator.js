...
export async function createSkillInteractive(projectPath) {
  ...
  if (existsSync(skillDir)) {
    logger.warn(`La skill "${answers.name}" ya existe en ${skillDir}`);
  ...
  logger.success(`Skill creada: ${skillPath}`);
  logger.plain('\nPróximos pasos:');
  logger.plain('  1. Editá el contenido de la skill');
  logger.plain('  2. Ejecutá: node src/cli.js skill-sync');
  logger.plain('  3. La skill aparecerá en AGENTS.md automáticamente\n');
