...
export async function generateFromWizard(targetPath, selections) {
  ...
  // Configurar MCP servers automáticamente
  await setupMCPFromWizard(targetPath, selections);
...
export async function generateEnvironment(targetPath, projectInfo) {
  // Determinar template específico según tipo detectado
  const templateName = selectTemplateForProject(projectInfo);
  
  const config = {
    projectName: getProjectName(targetPath),
    description: generateProjectDescription(projectInfo),
    level: 'intermediate',
    stack: generateStackDescription(projectInfo),
...
function selectTemplateForProject(projectInfo) {
  // Templates específicos por tipo detectado
  if (projectInfo.type === 'nextjs') return 'nextjs';
  if (projectInfo.type === 'api') return 'api';
  if (projectInfo.type === 'ai-agent') return 'ai-agent';
  if (projectInfo.type === 'web') return 'web';
  if (projectInfo.type === 'python') return 'python';
  return 'root';
}

function generateProjectDescription(projectInfo) {
  const descriptions = {
    'nextjs': 'Aplicación web con Next.js App Router',
    'api': 'API REST backend',
    'web': 'Aplicación web frontend',
    'python': 'Proyecto Python',
    'ai-agent': 'Sistema de agente IA con skills modulares',
    'go': 'Aplicación Go',
    'rust': 'Proyecto Rust',
  };
  return descriptions[projectInfo.type] || `Proyecto ${projectInfo.type}`;
}

function generateStackDescription(projectInfo) {
  const parts = [];
  
  if (projectInfo.language !== 'unknown') {
    parts.push(projectInfo.language.charAt(0).toUpperCase() + projectInfo.language.slice(1));
  }
  
  if (projectInfo.frameworks.length > 0) {
    parts.push(...projectInfo.frameworks.map(f => 
      f.charAt(0).toUpperCase() + f.slice(1)
    ));
  }
  
  return parts.join(', ') || 'Stack por definir';
}
...
export async function addSkill(targetPath, skillName) {
  ...
  if (!src) {
    logger.error(`Skill "${skillName}" no encontrado en templates`);
    return false;
  }
...
/**
 * Configura MCP servers basado en las selecciones del wizard
 */
async function setupMCPFromWizard(targetPath, selections) {
  try {
    const { setupMCP, detectRecommendedMCP } = await import('./mcp-generator.js');
    
    // Detectar MCP recomendados según selecciones
    const recommended = detectRecommendedMCP(null, selections);
    ...
    // Si falla MCP, no es crítico - continuar sin él
    logger.debug('MCP no configurado:', err.message);
  }
}
